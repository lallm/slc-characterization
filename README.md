...
Machine learning-based classification and unsupervised characterization of protein–protein interfaces (PPIs), distinguishing **transmembrane** from **soluble** interfaces. The study compares two feature representations — aggregated physicochemical features and Graph Neural Network (GNN) embeddings — across both supervised and unsupervised analysis pipelines. Structures are derived from experimentally validated PPIs predicted by AlphaFold3 and refined through molecular dynamics simulations.
...
pip install numpy pandas matplotlib scipy statsmodels seaborn scikit-learn joblib psutil ray
```
...
├── ppi.lib/         # Core Python library (add to sys.path before importing)
...
│   ├── vbgmm.py                   # Variational Bayes GMM implementation (parallelised via Ray)
│   ├── helpers.py                 # General data handling and preprocessing helpers
│   ├── genericfuncs.py            # Low-level utility functions
...
└── results/         # Computed outputs: cross-validation results, QIn matrices, GNN embeddings
```
...
The notebooks are intended to be executed in the order listed below.
...
| `clusterdata.ipynb` | Unsupervised clustering of interface archetypes using GMM and VBGMM on both aggregated features and GNN embeddings. Includes elbow analysis for determining the number of clusters. |
| `unsupervised_analysis.ipynb` | Kruskal-Wallis ε² effect size analysis across superclusters. Identifies which physicochemical features best discriminate the discovered interface archetypes. |
...
**Supervised analysis:** SVC, MLP, GPC and RF classifiers are trained on aggregated physicochemical features. A GNN is additionally trained on residue-level interface graphs, with the learned embeddings used as an alternative representation. Classification performance is evaluated using nested cross-validation.
...
**Unsupervised analysis:** GMM and VBGMM are applied to both feature representations to discover interface archetypes without using transmembrane labels. Soft cluster assignments (QIn matrices) are hierarchically agglomerated into superclusters using symmetric KL divergence. Kruskal-Wallis ε² effect sizes with Benjamini-Hochberg correction quantify feature discriminability across the discovered clusters.

---

## Input Data Format

The `source/` directory is expected to contain the following files:

| File | Description |
|---|---|
| `agg_final.csv` | Aggregated physicochemical feature matrix. One row per interface, with `ExpID` as identifier. |
| `agg_trans.csv` | Label file with `ExpID` and binary transmembrane label (`Transmembrane` or `y_true`). |
| `GNN_set2set_ALL_FOLDS.csv` | GNN embedding matrix (set2set pooling), one row per interface. |
...
Pre-computed outputs in `results/` include:
